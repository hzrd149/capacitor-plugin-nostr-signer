...
- Trust `src/index.ts` over the README/sample app for call signatures if they differ; the current wrapper exposes positional methods such as `signEvent(packageName, eventJson, id, pubkey)`, where `pubkey` is lowercase hex.
